...
const BroadcastDetail = ({ broadcast, onBack, isMobile, activeFilter, setActiveFilter, sortOrder, setSortOrder }) => {
  if (isMobile) {
      ...
      <div className="bg-tertiary rounded-t-3xl p-4 pb-6 space-y-4">
        ...
        <div className="bg-accent/5 rounded-xl p-4">
          <div className="prose prose-sm max-w-none">
            <p className="text-accent/80 whitespace-pre-wrap">{broadcast.content}</p>
            ...
            {broadcast.additionalInfo && (
              <div className="mt-4">
                <p className="text-accent/70 italic">{broadcast.additionalInfo}</p>
              </div>
            )}
          </div>
        </div>
        {broadcast.status === 'sent' && (
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-accent/5 rounded-xl p-3">
              ...
              <p className="text-2xl font-bold text-accent">{broadcast.recipients || 'All'}</p>
            </div>
          </div>
        )}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-tertiary rounded-2xl p-6 space-y-4">
        <div className="flex items-start justify-between">
          ...
          <div className="flex gap-2">
            <button className="p-2 hover:bg-accent/5 rounded-lg transition-colors">
      ...
      <div className="bg-tertiary rounded-2xl p-6">
        <div className="prose prose-lg max-w-none">
          ...
          {broadcast.additionalInfo && (
            <div className="mt-6">
              <p className="text-accent/70 italic text-base">{broadcast.additionalInfo}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {broadcast.status === 'sent' && (
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="bg-tertiary rounded-2xl p-6">
            ...
            <p className="text-3xl font-bold text-accent">{broadcast.recipients || 'All'}</p>
          </div>
          <div className="bg-tertiary rounded-2xl p-6">
            <p className="text-sm text-accent/60 mb-2">Priority</p>
            <Badge className={`${broadcast.priorityColor} border-0 text-sm px-3 py-1`}>
              {broadcast.priority}
            </Badge>
          </div>
        </div>
